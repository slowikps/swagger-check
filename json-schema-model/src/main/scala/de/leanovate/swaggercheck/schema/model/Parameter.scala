...
object Parameter {
  def build(
    ...
    if (in == "body") Parameter(name, in, required.getOrElse(false), schema.get)
      ...
      case Some("array") => Parameter(name, in, required.getOrElse(false), ArrayDefinition(minItems, maxItems, items, false))
      case Some("string") => Parameter(name, in, required.getOrElse(false), StringDefinition(format, minLength, maxLength, pattern, enum, false))
      case Some("integer") => Parameter(name, in, required.getOrElse(false), IntegerDefinition(format, minimum.map(_.longValue()), maximum.map(_.longValue()), false))
      case Some("number") => Parameter(name, in, required.getOrElse(false), NumberDefinition(format, minimum.map(_.doubleValue()), maximum.map(_.doubleValue()), false))
      case Some("boolean") => Parameter(name, in, required.getOrElse(false), BooleanDefinition(false))
    }
  }
}